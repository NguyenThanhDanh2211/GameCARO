...
class PlaywAI_EASY:
    ...
    def save_game_state(self, current_round, human_score, com_score):        
        ...
        existing_state = easy_collection.find_one({"player_name": self.player_name})
        if existing_state:
            # Nếu đã tồn tại, cập nhật thông tin
            easy_collection.update_one({"player_name": self.player_name}, {"$set": game_state})
        else:
    ...
    def count_round_completed(self, current_round, human_score, com_score):
        if current_round > 5:
            ...
            final_scores_collection_easy.update_one({"player_name": self.player_name}, {"$set": final_scores_easy}, upsert=True)

    def load_game_state(self):
        game_state = easy_collection.find_one({"player_name": self.player_name})
